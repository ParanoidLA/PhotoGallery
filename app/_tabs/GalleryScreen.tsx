...
const GalleryScreen = () => {
  const route = useRoute<RouteProp<{ params: { images: PixabayImage[], initialIndex: number } }, 'params'>>();
  ...
  const renderItem = ({ item }: { item: PixabayImage }) => {
    ...
    return (
      <TouchableOpacity onPress={() => handleDoubleTap(item.largeImageURL)} style={styles.imageContainer} activeOpacity={1}>
        <Image source={{ uri: item.largeImageURL }} style={styles.image} />
        <TouchableOpacity 
          style={styles.heartIcon} 
          onPress={() => isLiked ? removeLikedPhoto(item.largeImageURL) : addLikedPhoto(item.largeImageURL)}
        >
